...
if (isset($_POST['submit'])) {
    ...
    if (!$stmt->execute()) {
        ...
        // ===== Handle City =====
        $city_q = mysqli_query($con, "SELECT city_id FROM city WHERE city_name='$city_name'");
        if (mysqli_num_rows($city_q) > 0) {
            $city_row = mysqli_fetch_assoc($city_q);
            $city_id = $city_row['city_id'];
        } else {
            $stmt_city = $con->prepare("INSERT INTO city(city_name) VALUES (?)");
            $stmt_city->bind_param("s", $city_name);
            $stmt_city->execute();
            $city_id = $stmt_city->insert_id;
            $stmt_city->close();
        }
        ...
        if (!$stmt->execute()) {
            ...
            $stmt->bind_param(
                "iiiisiisi",
                $user_id, $agent_id, $vehicle_id, $service_id, $city_id, $created_at, $status, $landmark, $user_location_id
            );

            if ($stmt->execute()) {
                // ✅ Redirect to service-history.php after submission
...
.card-box { background: #fff; padding: 25px 30px; border-radius: 14px; box-shadow: 0 10px 25px rgba(0,0,0,0.08); }
.card-box h4.header-title { font-size: 20px; font-weight: 600; margin-bottom: 20px; color: #0f172a; }
